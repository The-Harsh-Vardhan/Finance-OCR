...
export default async function handler(req: Request) {
  ...
  if (req.method === 'GET') {
    return new Response(
      JSON.stringify({
        status: 'online',
        service: 'GramIQ Finance OCR Standalone API',
        version: '3.5-SOTA',
        endpoint: '/api/ocr',
        primary_engine: 'GCP Vertex AI (gemini-3.5-flash-lite)',
        fallback_engine: 'Google AI Studio (gemini-3.5-flash-lite)'
      }),
      {
        status: 200,
  ...
  try {
    ...
    if (!apiKey && !saJsonRaw) {
      return new Response(
        JSON.stringify({ error: 'No GEMINI_API_KEY or GCP_SERVICE_ACCOUNT_JSON configured in environment.' }),
    ...
    // 1. Service Account GCP Vertex AI Priority
    ...
    for (const ep of endpointsToTry) {
      ...
      try {
        ...
        if (res.ok) {
          ...
          attempts.push({ engine: ep.name, status: 'FAILED', error: `HTTP ${res.status}` });
          console.warn(`Endpoint ${ep.name} failed [HTTP ${res.status}]: ${errBody.slice(0, 100)}`);
        }
      } catch (e: any) {
        attempts.push({ engine: ep.name, status: 'FAILED', error: e.message });
        console.warn(`Endpoint ${ep.name} exception: ${e.message}`);
      }
    }
    ...
    if (!geminiResData) {
      return new Response(
        JSON.stringify({
          error: 'All Gemini Vision endpoints failed',
          attempts,
          saAuthError
        }),
    ...
    // Resilient JSON parsing & bracket-balancing repair
    ...
    return new Response(
      JSON.stringify({ error: err.message || 'Internal server error during OCR processing' }),
